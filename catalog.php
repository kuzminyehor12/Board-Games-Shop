    ...
    <div class="header-wrapper">
        <div class="sort-container">
            ...
            <div>
                ...
                <select id="sort">
                    <option name="no_sort" value="-1"></option>
                    <option name="abc_sort" value="0">по алфавиту</option>
                    <option name="price_sort" value="1">цены по возрастанию</option>
                </select>
            </div>
        </div>
        <div class="catalog_container">
            <div class="sidebar">
                <?php
                echo "<p>Основные:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Основные'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Актуальные:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Актуальные'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Возраст участников:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Возраст участников'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Количество игроков:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Количество игроков'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Жанры:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Жанры'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Тематика:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Тематика'");
                print_to_list($categories);
                echo "<br>";

                echo "<p>Режим игры:</p><br>";
                $categories = query_to_array("SELECT * FROM `categories` WHERE `type` = 'Режим игры'");
                print_to_list($categories); 
                echo "<br>";

                echo "<p>Бренды:</p><br>";
                $brands = query_to_array("SELECT * FROM `brands`");
                print_to_list($brands);
                echo "<br>";
                ?>
            </div>
            <div class="content">
                <?php 
                if (isset($_GET['submit']) && isset($_GET['text']) && !empty($_GET['text'])){
					require_once "php/search.php";
				} else {
					$goods = get_goods();
					print_good_card($goods, restrictionFounder($goods));
				}
                ?>
            </div>
        </div>
