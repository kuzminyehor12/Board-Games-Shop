<?php
    ...
    $goods = get_goods();
    $game = $goods[$_GET['id'] - 1];

    function print_sibling($array, $rest, $key){
        echo "<div class=\"sibling\"><div class=\"good-card\">
            <div class=\"good-img\">
            <img style = \"width: 100%;height: 100%\" src=\"goods_images/" . $array[$key]['image'] . ".jpg\" alt=\"\">
            </div> <div class=\"good-title\">
            <span>Артикул: " . $array[$key]['vendorCode'] . "</span><br>
            <span class=\"restriction\">" . $rest[$key] . "<br>
            <a href=\"good_page.php?id=". $array[$key]['vendorCode'] . "\">" . $array[$key]['title'] . "</a>
            </div>
            <div class=\"commerce-info\">
            <span style=\"font-size: 20px; font-weight: 700\" class=\"price\">" . $array[$key]['price'] . " грн</span>
...
<head>
    ...
    <link rel="stylesheet" href="css/style_good_page.css">
        ...
        <div class="header-wrapper">
            <div class="good-page-container">
                <div class="board-game-info">
                    <?php
                        echo "<img style=\"width: 50%; height: 50%\" src=\"goods_images/" . $game['image'] . ".jpg\" alt=\"\">";
                        echo "<br>" . $game['short_description'];
                        echo "<br>" . $game['long_description'];
                    ?>
                </div>
                <div class="board-game-parametres">
                    <div class="parametres">
                        <?php
                            $categories = query_to_array("SELECT * FROM `categories` LEFT OUTER JOIN goodsncat ON goodsncat.categoryID = categories.id WHERE goodsncat.vendorCode = " . $game['vendorCode'] . "");

                            separate_categories($categories, "Основные");
                            separate_categories($categories, "Возраст участников");
                            separate_categories($categories, "Количество игроков");
                            separate_categories($categories, "Жанры");
                            separate_categories($categories, "Тематика");
                        ?>

                        <div class="commerce-info">
                            <span style="font-size: 25px; font-weight: 700; margin-left: 10px" class="price"><?echo $game['price']?> грн</span>
                            <button class="buying-btn">Купить</button>
                        </div>
                    </div>
                    <div class="same-category-game">
                        <?php 
                            $same_category = query_to_array("SELECT * FROM `categories` LEFT OUTER JOIN goodsncat ON goodsncat.categoryID = categories.id WHERE goodsncat.vendorCode = " . $game['vendorCode'] . " AND categories.type = 'Жанры'");
                            
                            if(isset($same_category[1])){
                                $sibling = query_to_array("SELECT * FROM `goods` LEFT OUTER JOIN goodsncat ON goodsncat.vendorCode = goods.vendorCode WHERE goodsncat.categoryID IN (" . $same_category[1]['id'] . ")");
                            }

                            if(isset($sibling[1])){
                              print_sibling($sibling, restrictionFounder($sibling), 1);
                            }
                        ?>
                    </div>
                </div>
